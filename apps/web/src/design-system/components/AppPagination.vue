...
const props = defineProps<{
  ...
  page: number;
  pageCount: number;
  /** The nav landmark's accessible name, e.g. "Audit log pages". */
  label: string;
  prevLabel: string;
  nextLabel: string;
}>();
...
// First, last and the current page's neighbours; a gap that would hide a
// single page shows that page instead of an ellipsis.
...
<style scoped>
/* margin/display reset: the app's global `nav` rule adds a 32px top margin and a grid. */
