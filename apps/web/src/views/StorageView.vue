...
<template>
  <div class="view">
    <header class="view-header">
      <div>
        <h1>ストレージ</h1>
        <p class="subtitle">{{ mounts.length }}件のマウント</p>
      </div>
    ...
    <section class="table-panel storage-panel">
      <div class="table-scroll">
        <table class="storage-table">
          ...
          <tbody>
            <tr v-if="loading && mounts.length === 0"><td colspan="4" class="empty">ストレージ情報を読み込んでいます…</td></tr>
            <tr v-else-if="!loading && mounts.length === 0 && !error"><td colspan="4" class="empty">マウントされたファイルシステムはありません。</td></tr>
            <tr v-for="mount in mounts" :key="`${mount.filesystem}:${mount.mount_point}`">
              <td class="path-cell">
                <strong class="storage-path" :title="mount.mount_point">{{ mount.mount_point }}</strong>
              </td>
