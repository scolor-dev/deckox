...
<template>
  <div class="shell">
    <header class="mobile-bar">
      ...
      <button class="menu-button" type="button" :aria-expanded="menuOpen" aria-label="メニューを開く" @click="menuOpen = !menuOpen">
        {{ menuOpen ? "閉じる" : "メニュー" }}
      </button>
    </header>
    ...
    <aside :class="['sidebar', { open: menuOpen }]">
      ...
      <nav aria-label="メインナビゲーション">
        <button type="button" :class="{ active: activePage === 'overview' }" @click="navigate('overview')">概要</button>
        <button type="button" :class="{ active: activePage === 'services' }" @click="navigate('services')">サービス</button>
        <button type="button" :class="{ active: activePage === 'storage' }" @click="navigate('storage')">ストレージ</button>
      </nav>
      ...
      <div class="sidebar-footer">バージョン {{ status?.version ?? "0.1.0" }}</div>
    </aside>
