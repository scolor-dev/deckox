...
const DEFAULT_HOME: &str = "/var/lib/deckox";
...
impl TerminalManager {
    pub fn from_env() -> Result<Self, String> {
        ...
        if !ALLOWED_SHELLS
            .iter()
            .any(|allowed| shell.as_path() == Path::new(allowed))
        {
            return Err(format!(
        ...
        if requested && !enabled {
            warn!(
                ...
                reason = "root_process",
                "web terminal is disabled because deckox-server is running as root"
            );
        }
        Ok(Self {
...
pub async fn status(State(manager): State<TerminalManager>) -> Json<TerminalStatus> {
    Json(manager.status())
}
...
pub async fn websocket(
    ...
    upgrade
        ...
        .on_upgrade(move |socket| async move {
            let _permit = permit;
            run_terminal(socket, manager, request_id.0, user.source_ip).await;
        })
}
